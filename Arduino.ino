...
#define default_angle 125   // Straight Angle	// Default 125
...
void setup() {
  ...
  // 	Define Motors Pins
  pinMode(EnableL, OUTPUT);		//Setting Left Motor pins to OUTPUT
  pinMode(Left_1, OUTPUT);     
  pinMode(Left_2, OUTPUT);

  pinMode(EnableR, OUTPUT);		//Setting Right Motor pins to OUTPUT
...
{  
  ...
  if (distance < 30) //Checks for obstacles
  {
  ...
  else if (distance > 30 && !Park_spot_scan) // If an empty place spotted
  {
    initial_time = millis(); 
    Park_spot_scan = 1;
    Serial.println("Started_Park_scan");
  }
